  ...
  private def generateFringeColorMapping(
    ...
    val fringes = blocks
      .foldLeft(initFringeMap) { case (acc, b) =>
        // in case DAG is cut and not all blocks till genesis are present
        val fringePresent = b.fringe.forall(blocks.map(_.id).contains)
        if (fringePresent) {
