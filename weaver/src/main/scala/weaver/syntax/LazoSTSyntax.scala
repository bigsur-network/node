...
final class LazoSTOps[M, S](private val s: LazoState[M, S]) {
  ...
  def latestFringe(x: Set[M]): LazoF[M] =
    x.map(s.dagData(_: M).fringeIdx).maxOption.map(s.fringes).map(LazoF(_)).getOrElse(LazoF.empty[M])
  ...
  def finality(mgjs: Set[M]): LazoF[M] = Finality.tryAdvance(mgjs, s).getOrElse(latestFringe(mgjs))
